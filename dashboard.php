...
$stats = [];
if ($user_role === 'Buyer') {
    $orders_query = "SELECT COUNT(*) as total_orders, COALESCE(SUM(totalPrice), 0) as total_spent FROM `order` WHERE buyerID = ?";
    $orders_stmt = $db->prepare($orders_query);
    $orders_stmt->execute([$user_id]);
    $order_stats = $orders_stmt->fetch(PDO::FETCH_ASSOC);
    
    $cart_query = "SELECT COUNT(ci.cartItemID) as cart_items FROM cart c JOIN cartitem ci ON c.cartID = ci.cartID WHERE c.buyerID = ?";
    ...
    $products_query = "SELECT COUNT(*) as total_products, COUNT(CASE WHEN status = 'approved' THEN 1 END) as approved_products, COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending_products FROM product WHERE sellerID = ?";
    $products_stmt = $db->prepare($products_query);
    $products_stmt->execute([$user_id]);
    $stats = $products_stmt->fetch(PDO::FETCH_ASSOC);
} elseif (in_array($user_role, ['Admin', 'Moderator'])) {
    $admin_query = "SELECT (SELECT COUNT(*) FROM user) as total_users, (SELECT COUNT(*) FROM product WHERE status = 'pending') as pending_products, (SELECT COUNT(*) FROM `order`) as total_orders, (SELECT COUNT(*) FROM supportticket WHERE status = 'Open') as open_tickets";
