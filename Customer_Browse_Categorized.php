...
$category_name_sql = $db->prepare("SELECT Name FROM Categories WHERE categoryID = $category_id");
$category_name_sql->execute();
$cat_row  = $category_name_sql->fetch(PDO::FETCH_ASSOC);
...
// Decorator DP Implementation
    ...
    //Choosing Components based on page requirements
    $filter = new CategorizedFilter($category_id);

    // Look out for inputs from user. If not empty, then call respective Concrete Decorators.
    ...
    // Once we are done adding Filters, call the build function on $filter
